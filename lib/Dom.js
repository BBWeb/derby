...
function Dom(model) {
  ...
  function onDomTrigger(name, listener, id, e, el, next) {
    ...
    // Workaround for Chrome's recent addition of the read-only event.path property,
    // which meant meant that our e.path function below was never set.
    // By deleting properties explicitly we clear their read-only status and make sure they can be overwritten.
    // This essentially means this version of Derby can not be compatible with any script that uses the new
    // event.path property (or event.get / event.at if they are added at some point in the future)
    delete e.path;
    delete e.get;
    delete e.at;

    Object.defineProperty(e, 'path', { writable: true });
    Object.defineProperty(e, 'get', { writable: true });
    Object.defineProperty(e, 'at', { writable: true });
